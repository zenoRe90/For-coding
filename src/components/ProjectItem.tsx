      ...
      {inView ? (
        <div ref={contentRef} className="flex flex-col items-center justify-center w-full h-full">
          ...
          <div className="w-14 h-14 rounded-2xl bg-bg-main/50 flex items-center justify-center mb-2 group-hover:scale-105 transition-transform">
            <Folder className={cn("w-8 h-8 transition-colors", project.color || "text-text-muted group-hover:text-accent")} />
          </div>
          ...
          {projectTags.length > 0 && (
            <div className="flex gap-1 mt-2">
              {projectTags.map((tag, i) => tag && (
                <div key={i} className={cn("w-2.5 h-2.5 rounded-full shadow-sm", tag.color)} title={tag.name} />
              ))}
            </div>
          )}
        </div>
